...
##
## Begin tgp and deepgp helper functions
##
...
## optim.step.tgp.new
##
## updates tgp::optim.step.tgp to include a new cands="tri" option,
## otherwise it is the same as the old version

optim.step.tgp.new <- function (f, rect, model = btgp, prev = NULL, X = NULL, Z = NULL,
    NN = 20 * length(rect), improv = c(1, 5), cands = c("lhs", "tri", "tdopt"), 
    method = c("L-BFGS-B", "Nelder-Mead", "BFGS", "CG", "SANN", "optimize"), ...)
 {
  rect <- matrix(rect, ncol = 2)
  if(cands == "tri") {
    out <- model(X = X, Z = Z, ...)
    m <- which.min(out$Zp.mean)
    XX <- tricands(X, max=NN, best=m)
  } else XX <- tgp:::tgp.cands(prev$obj, NN, cands, rect, X)
...
## optim.step.tgp.simple
##
## stripped down version of optim.step.tgp to remove the extra optim
## steps, to offer a fairer comparison to deepGP alternatives

optim.step.tgp.simple <- function(f, model = btgp, X = NULL, Z = NULL, 
    NN = 20 * length(rect), cands = c("lhs", "tri"), cand_params, ...)
 {
  #if(cands == "tri") { 
  #  out <- model(X = X, Z = Z, ...)
  #  m <- which.min(out$Zp.mean)
  #  XX <- tricands(X, max=NN, best=m)
  #} else XX <- randomLHS(NN, ncol(X))
  XX <- get_cands(X=X, m=m, ncands=NN, cands=cands, cand_params = cand_params, y = y)

  out <- model(X = X, Z = Z, XX = XX, improv=TRUE, pred.n=FALSE, ...)
  m <- which.max(out$improv[,1])
...
## optim.step.deepgp.simple:
##
## deepgp pacakge analog to optim.step.tgp.simple

optim.step.deepgp.simple <- function(f,X = NULL, Z = NULL,
    NN = 20 * length(rect), cands = c("lhs", "tri"), cand_params=cand_params, ...)
{
  ...
  fit <- fit_two_layer(X, Z, nmcmc = 10000, ...)
  fit <- trim(fit, 5000, 2)
  fit <- predict(fit, XX, lite = TRUE, EI = TRUE)
