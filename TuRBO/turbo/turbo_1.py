...
from .vor_portal import vor_cands
...
class Turbo1:
    ...
    def _create_candidates(self, X, fX, length, n_training_steps, hypers):
        ...
        isin = np.zeros(N)
        for i in range(N):
            isin[i] = np.all(np.logical_and(X[i,:] >= lb, X[i,:]<=ub))
        self.isins.append(isin)
        self.lengths.append(length)

        # Draw a Sobolev sequence in [lb, ub]
        if self.vor_tr:
            #pert = vor_cands(X, fX, self.n_cand, onlylhs=True, lb=lb, ub=ub)
            isinarr = np.array(isin).astype(bool)
            pert = vor_cands(X[isinarr,:], fX[isinarr], self.n_cand, onlylhs=True, lb=lb, ub=ub)
        else:
